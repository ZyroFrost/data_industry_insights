...
def load_csv_to_db_optimized(conn, csv_file: str, table_name: str, id_column: str, 
                             columns: List[str], skip: bool = False, auto_overwrite: bool = False):
    ...
    if auto_overwrite:
        choice = 'y'
        print(f"   Tự động xóa và load lại (auto mode)")
    else:
...
def create_dim_countries_view(conn):
    """Tạo VIEW Dim_Countries từ bảng locations"""
    cursor = conn.cursor()
    try:
        log_progress("Đang tạo VIEW Dim_Countries...")
        ...
        log_progress("Đã tạo VIEW Dim_Countries thành công", "✅")
        return True
    except Exception as e:
        conn.rollback()
        cursor.close()
        log_progress(f"Lỗi tạo VIEW: {e}", "❌")
        return False
...
def main():
    ...
    print("\n" + "=" * 70)
    print("💡 CHỌN CÁCH LOAD")
    print("=" * 70)
    print("1 = Tự động ghi đè toàn bộ table")
    print("2 = Chọn ghi đè từng table")
    print("=" * 70)
    
    mode_choice = input("\nChọn mode (1/2): ").strip()
    
    auto_overwrite = (mode_choice == '1')
    ...
    for csv_file in parent_tables:
        if csv_file in TABLE_MAPPINGS:
            ...
            if load_csv_to_db_optimized(conn, csv_file, config["table"], 
                             ...
                             config.get("skip", False), auto_overwrite):
                success_count += 1
    ...
    for csv_file in junction_tables:
        if csv_file in TABLE_MAPPINGS:
            ...
            if load_csv_with_fk_mapping_optimized(conn, csv_file, config["table"], 
                                       config["columns"], config.get("skip", False), auto_overwrite):
                success_count += 1
    
    print("\n" + "=" * 70)
    print("📦 PHASE 4: Creating Views")
    print("=" * 70)
    create_dim_countries_view(conn)
