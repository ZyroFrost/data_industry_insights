...
RULE_BASED_MAP = {
    ...
    "department": ["department","team","org_unit"],
...
if export_clicked:
    ...
    if "__source_name" in INGESTION_SCHEMA:
        df_out["__source_name"] = selected_file.name

    if "__source_id" in INGESTION_SCHEMA:
        # nếu source có cột id map vào thì đã có rồi
        if "__source_id" not in df_out.columns:
            df_out["__source_id"] = NA_VALUE
