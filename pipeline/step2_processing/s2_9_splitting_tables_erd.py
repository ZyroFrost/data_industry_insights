...
This step splits the combined dataset into ERD-ready tables.
...
- job_postings.csv
    ...
    + posted_date, min_salary, max_salary, currency
    + required_exp_years, education_level, employment_type
    + job_description, remote_option

- skills.csv
    + skill_id (PK)
    + skill_name
    + skill_category (single source of truth from skill_mapping.csv)
...
- job_roles.csv
    + job_id (FK → job_postings)
    + role_id (FK → role_names)

- job_levels.csv
    + job_id (FK → job_postings)
    + job_level (enum)

Derived BI dimension:
- dim_countries.csv
    + country
    + country_iso
    + country_iso_lower
    + flag_url (string metadata only, no external fetch in pipeline)

Notes:
- All outputs are generated from local CSV processing.
- No database, SQL, or external API calls are used in this step.
- flag_url is stored as plain string metadata; images are fetched only
  at visualization / BI rendering time.
...
- STEP 2.9 does NOT audit / drop / count skill
"""
...
def run():
    ...
    dim_countries = []   # ✅ ADD (BI DIMENSION)
    ...
    for _, r in df.iterrows():
        ...
        if r["company_name"] not in company_map:
            ...
            companies.append({
                ...
                "company_size": r["company_size"] if r["company_size"] in COMPANY_SIZES else None,
        ...
        job_postings.append({
            ...
            "job_description": to_null(r["job_description"]),
    ...
    for loc in locations:
        ...
        if not country or not country_iso:
            continue
        ...
        dim_countries.append({
            "country": country,
            "country_iso": country_iso,
            "country_iso_lower": iso_lower,
    ...
    pd.DataFrame(dim_countries).to_csv(
        OUTPUT_DIR / "dim_countries.csv",
        index=False,
        encoding="utf-8-sig"
    )
    ...
    print(
        ...
        f"  - dim_countries             : {len(dim_countries)}"
    )
