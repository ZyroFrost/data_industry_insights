...
def normalize_currency(x, NA="__NA__"):
    ...
    v_raw = str(x).strip()
    if v_raw == "" or v_raw == NA:
        return NA

    # =========================
    # LOAD LOOKUP ONCE (FILE-BASED)
    # =========================
    if not hasattr(normalize_currency, "_lookup"):
        if not CURRENCY_MAP_PATH.exists():
            normalize_currency._lookup = {}
            normalize_currency._canonical = set()
        else:
            df = pd.read_csv(CURRENCY_MAP_PATH, dtype=str)
            ...
            normalize_currency._lookup = lookup
            normalize_currency._canonical = canonical_set

    lookup = normalize_currency._lookup
    canonical_set = normalize_currency._canonical

    # =========================
    # 0. ALREADY CANONICAL → KEEP
    # =========================
    if v_raw.upper() in canonical_set:
        return v_raw.upper()
...
def normalize_remote_option(x, NA="__NA__"):
    ...
    # already canonical enum → KEEP (case-insensitive)
    enum_map = {
        "onsite": "Onsite",
        "remote": "Remote",
        "hybrid": "Hybrid",
...
def normalize_employment_type(x, NA="__NA__"):
    ...
    v_raw = str(x).strip()
    if v_raw == "" or v_raw.upper() == NA:
        return NA

    # =========================
    # 0. ALREADY CANONICAL → KEEP (CRITICAL FIX)
    # =========================
    ...
    if v_raw in CANONICAL_EMPLOYMENT:
        return v_raw

    # =========================
    # 1. NORMALIZE & LOOKUP
    # =========================
    key = normalize_text(v_raw)
...
def normalize_company_size(x, NA="__NA__"):
    if pd.isna(x):
        return NA
    ...
    # =========================
    # 2. NUMERIC-FIRST NORMALIZATION
    # =========================
    nums = []
    for m in re.finditer(r"(\d+(?:\.\d+)?)(k)?", v):
        n = float(m.group(1))
        if m.group(2):
            n *= 1000
        nums.append(int(n))
...
def normalize_city_file(file_path: Path):
    ...
    # =========================
    # NORMALIZE COMPANY SIZE (ENUM)   <-- THÊM Ở ĐÂY
    # =========================
    company_size_norm_count = 0
    if "company_size" in df.columns:
        before_size = df["company_size"].copy()
        df["company_size"] = df["company_size"].apply(normalize_company_size)
        company_size_norm_count = (before_size != df["company_size"]).sum()
    ...
    print(
        ...
        f"    - Company size normalized     : {company_size_norm_count}\n"
