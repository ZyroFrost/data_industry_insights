...
def process_file(path: Path, fx_rates: dict):
    ...
    for i, row in df.iterrows():
        ...
        # ================================
        # FIX: PRIORITIZE YEAR OVER OTHERS
        # ================================

        UNIT_PRIORITY = ["year", "month", "week", "hour"]
        ...
        for unit in UNIT_PRIORITY:
            ...
            # YEARLY salary must fully fit in range
            if YEARLY_MIN <= y_min <= YEARLY_MAX and YEARLY_MIN <= y_max <= YEARLY_MAX:
                chosen_unit = unit
